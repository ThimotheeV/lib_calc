...
int combination(int k_element, int n_element)
{
    ...
    {
        throw std::logic_error("combination : can't compute a combination where k > n");
    }
...
void bin_vec::insert(int pos, bool value)
{
    if (pos >= Size)
    {
        std::string temp_str = "__n (which is " + std::to_string(pos) + ") >= this->size() (which is " + std::to_string(Size) + ")";
...
std::vector<bool> bin_vec::and_(bin_vec const &vec1, bin_vec const &vec2)
{
    ...
    {
        throw std::length_error("the two binnary vector haven't the same length");
    }
