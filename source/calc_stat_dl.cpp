...
std::vector<double> calc_phi_ij(data_plane_vec_c const &data_plane_vec)
{
    int locus_pair_nbr = (data_plane_vec.nbr_of_locus() * (data_plane_vec.nbr_of_locus() - 1)) / 2;

    std::vector<double> result;
    result.reserve(locus_pair_nbr);

    for (int locus_i = 0; locus_i < data_plane_vec.nbr_of_locus(); ++locus_i)
    {
        for (int locus_j = locus_i + 1; locus_j < data_plane_vec.nbr_of_locus(); ++locus_j)
        {
...
std::vector<std::array<double, 3>> calc_eta(data_plane_vec_c const &data_plane_vec)
{
    ...
    Q2_locus.reserve(data_plane_vec.nbr_of_locus());
...
std::vector<std::array<double, 3>> calc_eta_1_indiv_deme_v(data_plane_vec_c const &data_plane_vec)
{
    ...
    Q0_locus.reserve(data_plane_vec.nbr_of_locus());
    for (int locus = 0; locus < data_plane_vec.nbr_of_locus(); ++locus)
    {
        auto temp = calc_Q_inter_deme_per_locus(data_plane_vec, locus);
        Q2_locus.push_back(static_cast<double>(temp.at(0)) / temp.at(1));
